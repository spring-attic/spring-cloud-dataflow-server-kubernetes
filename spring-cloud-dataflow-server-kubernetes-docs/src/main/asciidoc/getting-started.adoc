...
+
```
dataflow:>stream deploy foo --platformName minikube  --packageVersion 1.0.0 --useSkipper
...
[NOTE]
====
While deploying the stream, we are supplying three new properties. 1) `--platformName` indicates the platform repository to 
use when deployed via Skipper 2) `--packageVersion` to which the package should confirm to 3) `--useSkipper` flag indicating 
to SCDF that the deployment should be delegated to Skipper.
====
...
[source,console,options=nowrap]
----
skipper:>history --release-name myfoo
╔═══════╤════════════════════════════╤════════╤════════════╤═══════════════╤════════════════╗
...
----
skipper:>manifest get --release-name myfoo
...
metadata:
  name: log
spec:
  resource: docker:springcloudstream/log-sink-rabbit:1.1.0.RELEASE
  resourceMetadata: docker:springcloudstream/log-sink-rabbit:jar:metadata:1.1.0.RELEASE
...
metadata:
  name: time
spec:
  resource: docker:springcloudstream/time-source-rabbit:1.2.0.RELEASE
  resourceMetadata: docker:springcloudstream/time-source-rabbit:jar:metadata:1.2.0.RELEASE
  applicationProperties:
    spring.metrics.export.triggers.application.includes: integration**
...
[NOTE]
====
Where `myfoo` is the release name generated by Skipper. This might change to `scdf-foo` or other naming patterns in the 
next milestone.
====
...
[source,console,options=nowrap]
----
dataflow:>stream update --name foo --packageVersion 1.0.0 --properties log.version=1.2.0.RELEASE
...
newly spawned `po/foo-log-v2-0-fjnlt` pod is bootstrapping. The version is incremented and the pod-name reflects the 
new name.
====
...
metadata:
  name: log
spec:
  resource: docker:springcloudstream/log-sink-rabbit:1.2.0.RELEASE
