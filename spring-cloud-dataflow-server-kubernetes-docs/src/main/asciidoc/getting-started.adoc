...
----
    spec:
      containers:
      - name: scdf-server
        image: springcloud/spring-cloud-dataflow-server-kubernetes:1.4.0.RELEASE   # <1>
        imagePullPolicy: Always
----
+
...
+
```
dataflow:>app import --uri https://bit.ly/Celsius-SR1-stream-applications-rabbit-docker
```
+
...
The _liveness_ and _readiness_ probes are using the _paths_ `/health` and `/info` respectively. They use a _delay_ of 10 for both and a _period_ of 60 and 10 respectively. You can chage these defaults when you deploy the stream by using deployer properties.
