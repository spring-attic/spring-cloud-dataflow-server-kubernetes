...
----
    spec:
      containers:
      - name: skipper
        image: springcloud/spring-cloud-skipper-server:1.0.5.RELEASE   # <1>
...
----
    spec:
      containers:
      - name: scdf-server
        image: springcloud/spring-cloud-dataflow-server-kubernetes:1.6.0.BUILD-SNAPSHOT   # <1>
        imagePullPolicy: Always
----
+
...
+
```
dataflow:>app import --uri https://bit.ly/Celsius-SR2-stream-applications-rabbit-docker
```
+
...
A Service Account for application deployments can be configured on a per app basis. An existing Service Account can be used, or a new one can be created. One way to create a service account is by using `kubectl`, for example:
...
Then applications can be configured as such:
...
NOTE: A Service Account name defined on a per application basis take precedence over a configured server `SPRING_CLOUD_DEPLOYER_KUBERNETES_DEPLOYMENT_SERVICE_ACCOUNT_NAME` environment variable.
