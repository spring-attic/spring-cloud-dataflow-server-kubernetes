...
----
    spec:
      containers:
      - name: scdf-server
        image: springcloud/spring-cloud-dataflow-server-kubernetes:1.4.0.RC1   # <1>
...
spec:
  ...
  template:
    ...
    spec:
      containers:
      - name: skipper
        image: springcloud/spring-cloud-skipper-server:1.0.1.RELEASE
        imagePullPolicy: Always
        ports:
        - containerPort: 80
        resources:
...
[source,console]
----
| SCDF-K8S-Server Version \ Chart Version | 0.1.0 | 0.2.0 |0.3.0 (future)|
|-----------------------------------------|-------|-------|--------------|
|1.2.x                                    |✓      |✕      |✕            |
|1.3.x                                    |✕      |✓      |✕            |
|1.4.x                                    |✕      |✕      |✓            |
