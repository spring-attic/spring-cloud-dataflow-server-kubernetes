...
The Spring Cloud Deployer that is being used has certain defaults for the memory and CPU setting that are used for deployed apps. With the defaults the container Limits and Requests are set to the following:
...
To set an environment variable for a deployed app use the `spring.cloud.deployer.kubernetes.environmentVariables` property like this:

```
deployer.<app>.kubernetes.environmentVariables=JAVA_TOOL_OPTIONS=-Xmx1024m
```

This example sets the `JAVA_TOOL_OPTIONS` environment variable to pass in a JVM memory setting. 
...
For setting values for the readiness probe just change the property name to include readiness instead of liveness.
