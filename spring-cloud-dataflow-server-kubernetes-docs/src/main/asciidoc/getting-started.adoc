...
[NOTE]
====
If your application is using Spring Boot 2.x+, all endpoints have been moved under `/actuator` by default. Here is an example of configuring Spring Boot 2.x+ _liveness_ and _readiness_ endpoint paths (just replace <app> with the name of your app):
