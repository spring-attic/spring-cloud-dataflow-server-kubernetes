...
A Service Account to use can be configured for all deployed applications. An existing Service Account can be used, or a new one can be created. One way to create a service account is by using `kubectl`, for example:
...
With the Service Account name, set the `SPRING_CLOUD_DEPLOYER_KUBERNETES_DEPLOYMENT_SERVICE_ACCOUNT_NAME` env variable, for example:
...
NOTE: A Service Account name defined on a per application basis takes precedence over this configuration setting.
...
An Image Pull Policy can be configured for all deployed applications, for example:
...
NOTE: An Image Pull Policy defined on a per application basis while deploying the stream or task takes precedence over this configuration setting.
