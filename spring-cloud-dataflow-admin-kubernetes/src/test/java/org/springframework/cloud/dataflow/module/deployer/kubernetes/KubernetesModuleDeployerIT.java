...
@SpringApplicationConfiguration(classes = org.springframework.cloud.dataflow.admin.spi.kubernetes.KubernetesModuleDeployerConfiguration.class)
public class KubernetesModuleDeployerIT {
	...
	public void end2endDeployment1() throws InterruptedException, ExecutionException  {
		...
		final Future<DeploymentState> future = executor.submit(new Callable<DeploymentState>() {
		    @Override
		    public DeploymentState call() throws Exception {
		    	while ( processModuleDeployer.status(id).getState() != DeploymentState.deploying ) {
					Thread.sleep(2000);
				}		    	
		...
		try {
			assertEquals(DeploymentState.deploying, future.get(15000, TimeUnit.MILLISECONDS));
